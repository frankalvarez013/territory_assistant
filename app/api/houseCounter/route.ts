...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { message: `houseCounter GET Transaction failed:\n ${e}` },
      { status: 404 }
    );
  }
}
...
export async function DELETE(
  request: NextRequest
): Promise<NextResponse<HouseCounter | ErrorResponse>> {
  const territoryID = request.nextUrl.searchParams.get("terrId");
  const congregationID = request.nextUrl.searchParams.get("congId");
  const territoryIDCheck = territoryID ? territoryID : undefined;
  ...
  if (territoryIDCheck && congregationID) {
    try {
      ...
      return NextResponse.json(
        { message: `houseCounter Delete Transaction failed:\n ${e}` },
        { status: 404 }
      );
    }
  }
